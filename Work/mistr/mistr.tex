...
\title{mistr: A Computational Framework for Mixture and Composite Distributions}
\author{by Lukas Sablica, Kurt Hornik}
...
useful theories how to describe financial markets. Some of these
...
distribution describing our behavior in these situations can change. A
simple illustration might be the log-returns distribution of the German
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-1-1} 
\includegraphics{mistr_files/figure-latex/unnamed-chunk-1-2} \end{Schunk}
...
and mixture models, where one assumes that the distribution is a finite
...
distributions cannot be used for modeling of the left tail.
...
classes for distributions, like many similar packages, to the best of
our knowledge it does not support any tools to work with composite
distributions. In particular, the only packages available for composite
models are the \CRANpkg{CompLognormal} package \citep{2b} and the
package \CRANpkg{gendist} \citep{gendist}, which can only deal with
two-components composite distributions. Whereas \pkg{distr} provides an
...
\end{Sinput}
\begin{Soutput}
#> [1]  2.4479758 -0.4671616 -0.3864586
\end{Soutput}
\end{Schunk}
...
logarithm, exponential and monotonic power transformations) and is
...
If the transformation is necessary, the transformation dispatches on the
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-9-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-10-1} \end{Schunk}
...
distribution are compared with a sample, a confidence bound for this
data is offered. This confidence ``envelope'' is based on the asymptotic
results of the order statistics. For a distribution \(F\) as \(n\) tends
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-11-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-14-1} \end{Schunk}
...
later for composite distributions.

Besides the quantile function and other main functions for evaluation,
...
\end{Sinput}
\begin{Soutput}
#> [1]  2.257775 -1.799452  2.227368 -1.162821
\end{Soutput}
\end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-17-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-20-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-21-1} \end{Schunk}
...
\end{Sinput}
\begin{Soutput}
#> [1] -4.219721 -2.143614 -4.467917 -3.392713
\end{Soutput}
\end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-24-1} \end{Schunk}
...
we created in the last section to compose a composite distribution with
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-27-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-28-1} \end{Schunk}
...
generality) of the framework, in this last section we will focus on
...
\] \[
 ...
 \] Due to the fact that a composite distribution can be represented as
...
breakpoints are known, the weights can be computed as \[
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-30-1} \end{Schunk}
...
\includegraphics{mistr_files/figure-latex/unnamed-chunk-32-1} \end{Schunk}
...
current list of known distributions and transformations by letting the
...
Lukas Sablica\\
Affiliation\\
Institute for Statistics and Mathematics, WU Wirtschaftsuniversität
Wien, Austria; \url{https://www.wu.ac.at/en/statmath}\\
}
